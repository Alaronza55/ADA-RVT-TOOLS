...
__doc__ = """Select a Detail Line in the active view."""
            ...
            for i, (tag, orig_head, tag_ref, tagged_elem_point, original_y) in enumerate(tags_sorted):
                ...
                if hasattr(tag, 'TagHeadPosition'):
                    ...
                    if tag_ref and tagged_elem_point:
                        try:
                            # Elbow should be at: X from tagged element, Y from tag head position (not new_y)
                            tag_head_y = tag.TagHeadPosition.Y
                            elbow_position = DB.XYZ(tagged_elem_point.X, tag_head_y, current_z)
