...
Places multiple dimensions from one detail line to another
"""
__title__ = "Auto-Dim\nDetail lines"
__author__ = "ADA"
...
Places dimensions between adjacent detail lines
"""
__title__ = "Auto-Dim\nDetail lines"
__author__ = "ADA"
...
def create_dimensions_between_lines():
    try:
        ...
        with revit.Transaction("Create Dimensions Between Lines"):
            ...
            # Create separate dimensions between adjacent lines
            for i in range(len(lines) - 1):
                # Create a reference array for just two adjacent lines
                pair_reference_array = ReferenceArray()
                pair_reference_array.Append(Reference(lines[i]))
                pair_reference_array.Append(Reference(lines[i + 1]))
                
                # Get the two lines
                line1 = lines[i].GeometryCurve
                line2 = lines[i + 1].GeometryCurve
                
                # Calculate the midpoint between the two lines
                midpoint1 = (line1.GetEndPoint(0) + line1.GetEndPoint(1)) / 2.0
                midpoint2 = (line2.GetEndPoint(0) + line2.GetEndPoint(1)) / 2.0
                pair_midpoint = (midpoint1 + midpoint2) / 2.0
                
                # Create dimension line point with offset
                pair_dim_point = pair_midpoint + offset_direction * offset_distance
                
                # Create dimension line
                pair_dim_line = Line.CreateBound(
                    pair_dim_point,
                    pair_dim_point + line_direction * 10.0  # Making line longer than needed
                )
                
                # Create dimension
                dim = doc.Create.NewDimension(active_view, pair_dim_line, pair_reference_array)
