...
def main():
    # Rotation correction (in radians) - adjust if needed
    # Positive = counter-clockwise, Negative = clockwise
    ROTATION_CORRECTION = math.pi / 2  # 90 degrees counter-clockwise
    
    # Define source family and type names
    source_family_name = "BES_RESA_CIRC"
    source_type_name = "BES_RESA CIRC VERTICAL"
        ...
        for i, instance in enumerate(source_instances):
            try:
                ...
                if isinstance(new_location, LocationPoint):
                    ...
                    total_rotation = old_rotation + ROTATION_CORRECTION
                    new_location.Rotate(axis, total_rotation)
                    print("  Rotation applied: original {:.3f} + correction {:.3f} = {:.3f} radians".format(
                        old_rotation, ROTATION_CORRECTION, total_rotation))
