...
def get_load_stat(cad, is_link):
    ...
    try:
        ...
        # If it's not a link, it's imported
        if not is_link:
            return "IMPORTED"
        
        # Try to get external file reference (only works for linked files)
        try:
            ...
            # If GetExternalFileReference fails, the element doesn't represent an external file
            # This can happen with imported (not linked) CAD files
            return "IMPORTED"
