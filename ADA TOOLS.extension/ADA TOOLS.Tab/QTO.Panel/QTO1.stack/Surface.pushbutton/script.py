...
Visualization: each picked face is recreated as a flat orange,
75%-opacity generic model (via TessellatedShapeBuilder, from
Face.Triangulate()), offset slightly off the real face so it doesn't
z-fight with it - the marker's own area matches the measured face's
area exactly, since it IS that face's shape. The area value (in
square meters) is also built as real 3D geometry near the face's
centroid: each digit is drawn as a blocky, 7-segment/digital-display-
style numeral out of small raised boxes (real Model Text turned out
to require creating a whole temporary family document under the
hood, so this sidesteps that entirely).
"""
...
try:
    ...
    total_area = 0.0
    face_details = []
    markers = []      # list of (triangles_host, centroid_host, normal_host, area_m2)
    ...
    report.line("Total surface area: <b>{:.3f} m2</b>".format(total_area_m2))
